  ...
  if( isset($testPath) ) {
    ...
    if (isset($options["run"]) && $options["run"]) {
      if (!strcasecmp($options["run"],'median')) {
        $raw = loadAllPageData($testPath, array('allEvents' => true));
      ...
      if( isset($options['cached']) ) {
        $pageData = loadPageRunData($testPath, $run, $options['cached'], array('allEvents' => true, 'allLevels' => true));
        ...
        if (!isset($pageData[$run][0]))
          unset($pageData);
        $pageDataRepeatedView = loadPageRunData($testPath, $run, 1, array('allEvents' => true, 'allLevels' => true));

        # merge data from first- and repeated-view
        foreach($pageDataRepeatedView as $eventName => $runData){
          foreach($runData as $runNumber => $cachedData) {
            foreach($cachedData as $cached => $data) {
              $pageData[$eventName][$runNumber][$cached] = $cachedData;
            }
          }
        }
      }
...
*/
function BuildHAR(&$pageDataArray, $id, $testPath, $options) {
  $result = array();
  $entries = array();
  ...
  foreach ($pageDataArray as $eventName => $eventData) {
    foreach ($eventData as $run => $pageRun) {
      foreach ($pageRun as $cached => $data) {
        ...
        $pd['id'] = "page_{$run}_{$cached}_{$data['eventNumber']}";
        ...
        if (isset($options['bodies']) && $options['bodies']) {
          $bodies_file = $testPath . '/' . $run . $cached_text . '_' . $data['eventNumber'] . '_bodies.zip';
