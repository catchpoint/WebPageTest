...
function getRequestCount(eventName){
    if (eventName != -1) {
...
function initDialog(eventName) {
    ...
    var CloseRequestDialog = function (hash) {
        ...
        $("#request-details" + eventNameID).show();
    }

    // initialize the pop-up dialog
    $('#request-dialog' + eventNameID).jqm({overlay: 0, onHide: CloseRequestDialog})
        .jqDrag('.jqDrag');
    $('input.jqmdX')
        .hover(function () {
    ...
    $("#request-dialog-radio" + eventNameID).change(function () {
        var panel = $('#request-dialog-radio' + eventNameID + ' input[type=radio]:checked').val();
        $("#dialog-contents" + eventNameID + " div.dialog-tab-content").hide();
        $("#" + panel).show();
    });
}
...
function SelectRequest(eventName, request) {
    ...
    $('#request-dialog' + eventNameID).css('top', $("#request-overlay-" + eventNameID + "-" + request).position().top + 20);
    $("#dialog-title" + eventNameID).html('<a href="#request' + eventNameID + '-' + request + '">Request #' + request + '</a>');
    var details='';
    var requestHeaders='';
    var responseHeaders='';
    ...
    if (wptRequests[request - 1] !== undefined) {
        ...
        if (r['body_url'] !== undefined && r['body_url'].length) {
                ...
                wptBodyRequest.onreadystatechange = function() {
                    if (wptBodyRequest.readyState == 4) {
                        if (wptBodyRequest.status == 200) {
                            $("#response-body" + eventNameID).text(wptBodyRequest.responseText);
                        } else {
            ...
            if (wptNoLinks) {
                $("#response-body" + eventNameID).html('<img style="max-width:100%; max-height:100%;" src="' + r['full_url'] + '">');
            } else {
                $("#response-body" + eventNameID).html('<a href="' + r['full_url'] + '"><img style="max-width:100%; max-height:100%;" src="' + r['full_url'] + '"></a>');
            }
        } else {
            $("#response-body" + eventNameID).html('Not Available.<br><br>Turn on the "Save Response Bodies" option in the advanced settings to capture text resources.');
        }
    }
    $("#request-details" + eventNameID).html(details);
    $("#request-headers" + eventNameID).html(requestHeaders);
    $("#response-headers" + eventNameID).html(responseHeaders);
