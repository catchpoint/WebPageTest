...
class LCP implements OpportunityTest
{
    public static function run(array $data): TestResult
    {
        ...
        if (isset($events) && is_array($events)) {
            ...
            foreach ($events as $event) {
                if (
                    isset($event['event'])
                    && $event['event'] == 'LargestContentfulPaint'
                    && isset($event['time'])
                    && isset($event['size'])
                ) {
        ...
        if (isset($lcp)) {
            if ($lcp['time'] > 2500) {
                ...
                if (isset($lcp['element']['outerHTML'])) {
                    ...
                    $hasHighFetchPriority = preg_match(
                        '/fetchpriority\="?high"?/i',
                        $lcp['element']['outerHTML']
                    ) === 1;
                }
                ...
                if (isset($lcp['type']) && $lcp['type'] === "image") {
                    if (isset($lcp['element']['nodeName']) && $lcp['element']['nodeName'] == 'VIDEO') {
                        $isVideo = true;
                        $lcpSource = isset($lcp['element']['poster'])
                          ? $lcp['element']['poster']
                          : $lcp['element']['src'];
                        ...
                        $lcpSource = isset($lcp['element']['src'])
                        ? $lcp['element']['src']
                        : $lcp['element']['currentSrc'];
                        $lcpFullSource = isset($lcp['element']['currentSrc'])
                        ? $lcp['element']['currentSrc']
                        : $lcp['element']['src'];
                        // get actual source attribute if we can.
                    ...
                    $expsToAdd[] = (object) [
                        ...
                        "desc" => '<p>This experiment adds a <code>&lt;link rel="preload" as="image" href="' .
                                  $lcpSource .
                                  '"&gt;</code> to the <code>head</code> of your HTML document, causing browsers to' .
                                  ' request the image earlier and at a higher priority than it otherwise might.</p>',
                        "expvar" => 'addpreload',
                    ...
                    if ($suggest_priority_hints) {
                        $expsToAdd[] = (object) [
                            ...
                            "desc" => '<p><em>(Chromium-only)</em> This experiment adds an <code>fetchpriority="high"' .
                                      '</code> attribute to your LCP image, causing it to request earlier at a higher' .
                                      ' priority.</p>',
                    ...
                    $opp = new TestResult([
                        ...
                        "desc" =>  "The element driving your LCP is a" . ($isBG ? " background" : "n")  . " image." .
                                   " Some optimizations can help that image fetch earlier.",
                        "examples" =>  array(
                    ...
                    $opp = new TestResult([
                        ...
                        "experiments" =>  array(
                            (object)[
                                ...
                                "desc" => '<p>When LCP is not image-based, that often means something is preventing' .
                                ' text from appearing sooner. Text visibility may be delayed by blocking' .
                                ' scripts or stylesheets, JavaScript-generated content, non-progressive custom font' .
