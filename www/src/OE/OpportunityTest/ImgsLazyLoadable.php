...
class ImgsLazyLoadable implements OpportunityTest
{
    public static function run(array $data): TestResult
    {
        ...
        if (isset($imgsOutViewport)) {
            foreach ($imgsOutViewport as $img) {
                if (
                    $img["loading"] !== "lazy" &&
                    !in_array($img["src"], $imgsInViewportSrcs) &&
                    strpos($img["src"], 'data:') !== 0
                ) {
        ...
        if (count($imgsThatShouldBeLazy)) {
            ...
            $opp = new TestResult([
                ...
                "desc" => "When images are lazy-loaded using loading=\"lazy\", when they scroll into the viewport," .
                          " freeing up early load for other tasks.",
                "examples" => array_unique($shouldBeLazySrcs),
                "experiments" => array(
                    (object) [
                        "id" => '014',
                        'title' => 'Add loading="lazy" to images',
                        "desc" => '<p>This experiment adds <code>loading="lazy"</code> attributes to images that are' .
                                  ' outside the viewport at load.</p>',
