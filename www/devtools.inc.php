                ...
                foreach ($rawRequests as &$rawRequest) {
                    if (array_key_exists('url', $rawRequest)) {
                        ...
                        if (
                            ...
                            $request['load_ms'] = -1;
                            if (
                                isset($rawRequest['response']['timing']['sendStart']) &&
                            ...
                            if (isset($rawRequest['response']['timing'])) {
                                ...
                                if (
                                    ...
                                    if (
                                        isset($rawRequest['response']['timing']['dnsStart']) &&
                                        ...
                                        if (!array_key_exists($dnsKey, $dnsTimes)) {
                                            ...
                                            if (
                                                isset($rawRequest['response']['timing']['dnsEnd']) &&
                                    ...
                                    if (
                                        isset($rawRequest['response']['timing']['connectStart']) &&
                                        ...
                                        if (
                                            isset($rawRequest['response']['timing']['connectEnd']) &&
                                    ...
                                    if (
                                        isset($rawRequest['response']['timing']['sslStart']) &&
                                        ...
                                        if (
                                            isset($rawRequest['response']['timing']['sslEnd']) &&
                ...
                if (isset($request['full_url'])) {
                  // Find the first matching request in the netlog
                    foreach ($netlog as $index => $entry) {
                        if (
                            isset($entry['url']) && isset($entry['start']) && !isset($entry['claimed']) &&
                ...
                if (
                    ...
                    if (array_key_exists($id, $rawRequests)) {
                        if (array_key_exists('redirectResponse', $event)) {
                            if (
                                !array_key_exists('endTime', $rawRequests[$id]) ||
                          ...
                          // so use that as an indicator.
                            if (
                                array_key_exists('fromDiskCache', $event['redirectResponse']) &&
                    ...
                    if (
                        ...
                        // so use that as an indicator.
                        if (
                            array_key_exists('fromDiskCache', $event['response']) &&
                        ...
                        if (
                            (!array_key_exists('bytesIn', $rawRequests[$id]) || !$rawRequests[$id]['bytesIn']) &&
                    ...
                    if ($event['method'] == 'Network.loadingFailed') {
                        if (
                            !array_key_exists('response', $rawRequests[$id]) &&
                            !isset($rawRequests[$id]['fromCache'])
                        ) {
    ...
    foreach ($rawRequests as &$request) {
        if (array_key_exists('startTime', $request)) {
            if (!isset($request['fromCache']) && isset($request['response']['timing'])) {
                if (
                    array_key_exists('requestTime', $request['response']['timing']) &&
            ...
            if (
                array_key_exists('startTime', $request) &&
                (!$pageData['startTime'] ||
...
function DevToolsGetConsoleLogForStep($localPaths)
{
    ...
    if (gz_is_file($console_log_file)) {
        ...
        $startOffset = null;
        if (
            GetDevToolsEventsForStep('Console.messageAdded', $localPaths, $events, $startOffset) &&
            is_array($events) &&
            count($events)
        ) {
            foreach ($events as $event) {
                  ...
                  // calculate the start time stuff
                    if ($method_class === 'Timeline') {
                        ...
                        if (
                            ...
                            if (
                                strpos($encoded, '"type":"Rasterize"') !== false ||
