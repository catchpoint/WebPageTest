...
return;var effectiveSelector=selectors[i].replace(pseudoSelectorRegexp,"");domModel.querySelector(document.id,effectiveSelector,queryCallback.bind(null,i===selectors.length-1?selectorsCallback.bind(null,styleSheets):null,selectors[i]));}}
...
doneCallback();for(var i=0;nodeIds&&i<nodeIds.length;++i){cssModel.getMatchedStylesAsync(nodeIds[i],false,false,matchedCallback);cssModel.getInlineStylesAsync(nodeIds[i],inlineCallback);cssModel.getComputedStyleAsync(nodeIds[i],imageStylesReady.bind(null,nodeIds[i],targetResult,i===nodeIds.length-1));}}
function onDocumentAvailable(root)
{if(progress.isCanceled())
return;domModel.querySelectorAll(root.id,"img[src]",getStyles);}
if(progress.isCanceled())
return;domModel.requestDocument(onDocumentAvailable);},__proto__:WebInspector.AuditRule.prototype}
...
function onDocumentAvailable(root)
{if(progress.isCanceled())
return;domModel.querySelectorAll(root.id,"body style",inlineStylesReceived.bind(null,root));}
...
function onDocumentAvailable(root)
{if(progress.isCanceled())
return;domModel.querySelectorAll(root.id,"head script[src] ~ link[rel~='stylesheet'][href]",lateStylesReceived.bind(null,root));}
domModel.requestDocument(onDocumentAvailable);},__proto__:WebInspector.AuditRule.prototype}
