...
static async startApplication(appName){console.timeStamp('Runtime.startApplication');if(typeof WorkerGlobalScope==='undefined')
Runtime.registerGlobalErrorHandlers();const allDescriptorsByName={};for(let i=0;i<allDescriptors.length;++i){const d=allDescriptors[i];allDescriptorsByName[d['name']]=d;}
...
static _shouldShowException(){const isConsoleDefined=typeof Common!=='undefined'&&typeof Common.console!=='undefined';return isConsoleDefined&&Runtime.experiments.isEnabled('uncaughtException');}
static _convertToError(event){if(event.error instanceof Error)
return event.error;const error=new Error(event.message);error.stack=`Error: ${error.message} \n at unknown (${event.filename}:${event.lineno}:${event.colno})`;return error;}
static _getSourceUrlForErrorReport(){return self.location.origin+self.location.pathname;}
static _handleUnhandledJavascriptError(event){const error=Runtime._convertToError(event);InspectorFrontendHost.reportError('UnhandledJavascriptError',error.message,error.stack,event.filename,Runtime._getSourceUrlForErrorReport(),event.lineno,event.colno);if(Runtime._shouldShowException())
Common.console.error(error.stack);Host.userMetrics.recordUnhandledJavascriptError(Host.UserMetrics.UnhandledJavascriptError.GenericException);}
static _convertPromiseRejectionToError(event){const reason=event.reason;if(reason instanceof Error)
return reason;const error=new Error(String(reason));error.stack=`Error: ${error.message} \n at unknown (unknown:0:0)`;return error;}
static _handleUnhandledPromiseRejection(event){const error=Runtime._convertPromiseRejectionToError(event);InspectorFrontendHost.reportError('UnhandledPromiseRejection',error.message,error.stack,'unknown',Runtime._getSourceUrlForErrorReport(),0,0);if(Runtime._shouldShowException())
Common.console.error(error.stack);Host.userMetrics.recordUnhandledJavascriptError(Host.UserMetrics.UnhandledJavascriptError.PromiseRejection);}
static registerGlobalErrorHandlers(){self.addEventListener('error',Runtime._handleUnhandledJavascriptError);self.addEventListener('unhandledrejection',Runtime._handleUnhandledPromiseRejection);}
...
allConfigurableExperiments(){const result=[];for(let i=0;i<this._experiments.length;i++){const experiment=this._experiments[i];if(!this._enabledTransiently[experiment.name]&&experiment.isConfigurable)
...
register(experimentName,experimentTitle,hidden,isConfigurable){Runtime._assert(!this._experimentNames[experimentName],'Duplicate registration of experiment '+experimentName);this._experimentNames[experimentName]=true;this._experiments.push(new Runtime.Experiment(this,experimentName,experimentTitle,!!hidden,isConfigurable));}
